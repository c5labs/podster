<?php
/**
 * Podster Controller File.
 *
 * @author   Oliver Green <oliver@c5labs.com>
 * @license  See attached license file
 */

namespace Concrete\Package\Podster\Block\PodsterPlayer;
...
class Controller extends BlockController
{
     ...
     * @var string
     */
    protected $btName = 'Podster Player';
     ...
     * @var string
     */
    protected $btDescription = 'Podcast player for the podster podcasting solution.';
     ...
     * @var string
     */
    protected $btHandle = 'podster_player';
     ...
     * @var string
     */
    protected $btTable = 'btPodsterPlayer';
    ...
    {
        // Find the episode.
        if (0 === intval($this->episodeID)) {
            $episode = $this->app->make('podster/repositories/episodes')->last($this->showID);
        } else {
            $episode = $this->app->make('podster/repositories/episodes')->find($this->episodeID);
        }

        // Set the show and episode and related episodes.
        if ($episode) {
            $this->set('episode', $episode);

            if ($show = $this->app->make('podster/repositories/shows')->find($this->showID)) {
                $this->set('show', $show);   
            }

            $relatedEpisodes = $this->app->make('podster/repositories/episodes')->all($this->showID);
            $this->set('relatedEpisodes', $relatedEpisodes);
        }

        // Set invalid configuration marker if needed.
        if (empty($show) || empty($episode)) {
            $this->set('invalid_configuration', true);
        }

        // Inject the helper.
        $app = \Concrete\Core\Support\Facade\Application::getFacadeApplication();
        $this->set('hp', $app->make('podster/helper'));
    }
    ...
    {
        // Set the available shows.
        $shows = $this->app->make('podster/repositories/shows')->all();

        $availableShows = [];
        foreach ($shows as $show) {
            $availableShows[$show['id']] = $show['title'];
        }

        $this->set('availableShows', $availableShows);

        // Set the available episodes.
        $episodes = $this->app->make('podster/repositories/episodes')->all();

        $availableEpisodes = array_map(function($item) {
            $keys = array_flip(['id', 'title', 'showID']);
            return array_intersect_key($item, $keys);
        }, $episodes->fetchAll());

        $this->set('availableEpisodes', $availableEpisodes);
    }
